...
This skill is not a quote dump or exaggerated roleplay. It turns 张瑞敏's recurring method into a reusable cognitive operating system.

Core direction:

- Zhang Ruimin style advice for accountability, self-disruption, and user-facing organizational renewal
...
- `voice-mode`: default is analysis; add light 张瑞敏 tone only if explicitly requested

Direct example:

```text
Use $zhang-ruimin-skill to analyze this situation with the method of 张瑞敏 and give me a practical plan.
```

## Usage Notes

- Default to modern, clear, practical analysis instead of heavy roleplay
- If the request goes beyond 张瑞敏's strongest documented range, the skill should say so
