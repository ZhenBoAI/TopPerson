...
This skill is not a quote dump or exaggerated roleplay. It turns 段永平's recurring method into a reusable cognitive operating system.

Core direction:

- Duan Yongping style advice for long-termism, selective focus, and business judgment
...
- `voice-mode`: default is analysis; add light 段永平 tone only if explicitly requested

Direct example:

```text
Use $duan-yongping-skill to analyze this situation with the method of 段永平 and give me a practical plan.
```

## Usage Notes

- Default to modern, clear, practical analysis instead of heavy roleplay
- If the request goes beyond 段永平's strongest documented range, the skill should say so
