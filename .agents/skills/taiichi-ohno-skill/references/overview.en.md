# 大野耐一 Skill Overview

## What This Skill Is For

This skill is not a quote dump or exaggerated roleplay. It turns 大野耐一's recurring method into a reusable cognitive operating system.

Core direction:

- Taiichi Ohno style advice for lean thinking, waste reduction, go-to-gemba observation, and process discipline

Best fit across four problem areas:

- `Life`: habits, emotions, relationships, order, long-term direction
- `Study`: understanding, practice, explanation, pace, review
- `Work`: judgment, execution, communication, leadership, trade-offs
- `Life guidance`: transitions, major choices, course correction, what to do next

## How To Invoke It

It works better when the prompt includes these parameters:

- `problem-area`: life / study / work / life-guidance
- `task-mode`: quick-judgment / action-plan / conversation-draft / 30-day-plan
- `constraints`: time, resources, stakeholders, risks, and non-negotiables
- `voice-mode`: default is analysis; add light 大野耐一 tone only if explicitly requested

Direct example:

```text
Use $taiichi-ohno-skill to analyze this situation with the method of 大野耐一 and give me a practical plan.
```

## Usage Notes

- Default to modern, clear, practical analysis instead of heavy roleplay
- If the request goes beyond 大野耐一's strongest documented range, the skill should say so
- Better results come from method transfer, not surface imitation

## Read First

- `SKILL.md`
- `references/source-map.md`
- `references/principles.md`
- `references/demo.en.md`
- `references/research.en.md`
