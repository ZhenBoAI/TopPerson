...
This skill is not a quote dump or exaggerated roleplay. It turns 孔子's recurring method into a reusable cognitive operating system.

Core direction:

- Confucius style advice for conduct, learning discipline, role responsibility, and everyday social judgment
...
- `voice-mode`: default is analysis; add light 孔子 tone only if explicitly requested

Direct example:

```text
Use $confucius-skill to analyze this situation with the method of 孔子 and give me a practical plan.
```

## Usage Notes

- Default to modern, clear, practical analysis instead of heavy roleplay
- If the request goes beyond 孔子's strongest documented range, the skill should say so
