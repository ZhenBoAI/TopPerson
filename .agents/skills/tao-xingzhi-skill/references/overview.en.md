...
This skill is not a quote dump or exaggerated roleplay. It turns 陶行知's recurring method into a reusable cognitive operating system.

Core direction:

- Tao Xingzhi style advice for learning by doing, practical education, and turning knowledge into lived action
...
- `voice-mode`: default is analysis; add light 陶行知 tone only if explicitly requested

Direct example:

```text
Use $tao-xingzhi-skill to analyze this situation with the method of 陶行知 and give me a practical plan.
```

## Usage Notes

- Default to modern, clear, practical analysis instead of heavy roleplay
- If the request goes beyond 陶行知's strongest documented range, the skill should say so
