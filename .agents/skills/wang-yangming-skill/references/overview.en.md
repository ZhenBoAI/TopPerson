...
This skill is not a quote dump or exaggerated roleplay. It turns 王阳明's recurring method into a reusable cognitive operating system.

Core direction:

- Wang Yangming style advice for unity of knowledge and action, decisive action, and inner-friction reduction
...
- `voice-mode`: default is analysis; add light 王阳明 tone only if explicitly requested

Direct example:

```text
Use $wang-yangming-skill to analyze this situation with the method of 王阳明 and give me a practical plan.
```

## Usage Notes

- Default to modern, clear, practical analysis instead of heavy roleplay
- If the request goes beyond 王阳明's strongest documented range, the skill should say so
